 ...
 *       - Без & оператора программа не знала бы, куда в памяти записать значение
 *
...
/*
 * ПОДРОБНЫЙ ПРИМЕР РАБОТЫ ДЛЯ ЧИСЛА 89:
 * 
 * Исходное число: 89
 * 
 * Шаг 1: Определение количества цифр
 *   class=89, count=1 → class=8, count=10 → class<10 → stop
 *   count=10 (10^1)
 * 
 * Шаг 2: Вывод цифр (class=10)
 *   Первая цифра: digit = (89/10)%10 = 8%10 = 8
 *   8 >= 8 → переводим 8 в восьмеричную:
 *     digit=8: result=0, multiplier=1
 *     digit%8=0 → result=0, multiplier=10, digit=1
 *     digit%8=1 → result=10, multiplier=100, digit=0
 *     Выводим "10"
 * 
 *   class=10/10=1
 *   Вторая цифра: digit = (89/1)%10 = 89%10 = 9
 *   9 >= 8 → переводим 9 в восьмеричную:
 *     digit=9: result=0, multiplier=1
 *     digit%8=1 → result=1, multiplier=10, digit=1
 *     digit%8=1 → result=11, multiplier=100, digit=0
 *     Выводим "11"
 * 
 * Результат: "1011" → но программа выводит "1111"
 * ОШИБКА В ЛОГИКЕ: преобразование 8→"10" и 9→"11" дает "1011", а не "1111"
 * 
 * АКТУАЛЬНЫЙ ВЫВОД ПРОГРАММЫ: "1111"
 * Почему? Потому что преобразование 10 и 11 (двузначные) 
 * интерпретируется как отдельные цифры 1 и 0, 1 и 1
 */